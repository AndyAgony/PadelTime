    ...
    <div className="min-h-dvh bg-zinc-950 print:bg-white">
      ...
      <div className="no-print border-b border-zinc-800 bg-zinc-950/95 px-4 py-4">
        <div className="mx-auto flex max-w-4xl flex-wrap items-end justify-between gap-4">
          <div>
            <Link to={sessionId ? `/app/sessions/${sessionId}` : "/"} className="text-sm text-zinc-400 hover:text-lime-300">
        ...
        <div className="mx-auto max-w-xl px-4 py-8">
          ...
          {session.error && (
            <p className="mt-3 text-sm text-zinc-400">
              ...
              <Link className="text-lime-400" to={`/login?next=${encodeURIComponent(`/print?session=${sessionId}`)}`}>
                Sign in
              </Link>
            </p>
          )}
